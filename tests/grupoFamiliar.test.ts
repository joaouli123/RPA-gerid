...
describe('validarGrupoFamiliar', () => {
  ...
  it('sinaliza ausência de Titular', () => {
  ...
  it('sinaliza mais de um Titular', () => {
    const gf = grupo('111.444.777-35', [
      integrante('João', 'Titular'),
      integrante('Outro', 'Titular'),
    ]);
    const motivos = validarGrupoFamiliar(gf, cliente('111.444.777-35'));
    expect(motivos.some((m) => m.detalhe.includes('Titular'))).toBe(true);
  });

  it('sinaliza CPF do Titular divergente do cliente', () => {
    const gf = grupo('111.444.777-35', [
      integrante('João', 'Titular', { cpf: '000.000.000-00' }),
    ]);
    const motivos = validarGrupoFamiliar(gf, cliente('111.444.777-35'));
    expect(motivos.some((m) => m.detalhe.includes('diverge'))).toBe(true);
  });
